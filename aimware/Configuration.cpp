...
void CConfig::Save()
{
	...
	if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_MYDOCUMENTS, NULL, 0, path)))
	{
		...
		folder = std::string(path) + ("\\AIMWARE\\");
		switch (g_Options.Menu.ConfigFile)
		{
		case 0:
			file = std::string(path) + ("\\AIMWARE\\") + std::string("Legit") + XorStr(".AIMWARE");
			break;
		case 1:
			file = std::string(path) + ("\\AIMWARE\\") + std::string("Rage") + XorStr(".AIMWARE");
			break;
		case 2:
			file = std::string(path) + ("\\AIMWARE\\") + std::string("HvH") + XorStr(".AIMWARE");
			break;
		case 3:
			file = std::string(path) + ("\\AIMWARE\\") + std::string("Other") + XorStr(".AIMWARE");
			break;
		}
