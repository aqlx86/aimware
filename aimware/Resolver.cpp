...
float CResolver::ResolveYaw(CBaseEntity* pPlayer, float org_yaw)
{
	...
	if (pPlayer->LowerBodyYaw() != LatestLowerBodyYaw[index] || (is_moving && is_onground))
	{
		...
		//we are storing in createmove, whatever
		//g_Resolver->StoreVars(pPlayer);
	...
	if (is_moving)
	{
		if (!is_onground)
		{
			switch (this->resolve_infos[index].stage)
			{
			case resolver_stages::lby_delta:
				new_yaw = org_yaw - YawDifference[index];
				break;
			case resolver_stages::at_bw:
				new_yaw = at_target.y + 180.f;
				break;
			case resolver_stages::at_left:
				new_yaw = at_target.y + 90.f;
				break;
			case resolver_stages::at_right:
				new_yaw = at_target.y - 90.f;
				break;
			case resolver_stages::at_half_left:
				new_yaw = at_target.y - 180 + 45.f;
				break;
			case resolver_stages::at_half_right:
				new_yaw = at_target.y - 180 - 45.f;
				break;
			default:
				break;
	...
	else /*On ground resolver*/
	{
		...
		else
		{
			...
			//new_yaw = LatestLowerBodyYaw[index] + Math::RandomFloat(-70, 70);
			//aa_info[index] = "AW";
		}
		//old good one
		/*if (abs(new_yaw - pPlayer->LowerBodyYaw()) < 5)
		{
			new_yaw -= 180;
			aa_info[index] = "Inverse";
		}
		else
		{
			...
			if (cur.size() > 1)
			{
				if (HasStaticRealAngle(cur, 5))
					...
					float tickdif = static_cast<float>(cur.front().tickcount - cur.at(1).tickcount);
					float lbydif = GetDelta(cur.front().m_flLowerBodyYawTarget, cur.at(1).m_flLowerBodyYawTarget);
					float ntickdif = static_cast<float>(G::UserCmdForBacktracking->tick_count - cur.front().tickcount);
					new_yaw = (lbydif / tickdif) * ntickdif;
				}
				else if (DeltaKeepsChanging(cur, 10))
					new_yaw -= GetDeltaByComparingTicks(cur, pPlayer);
